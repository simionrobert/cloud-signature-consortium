...
router.post('/signHash',
   passport.authenticate('bearer', { session: false }), function (req, res, next) {
      ...
      if (!validator.isBase64(hash)) { return next(errors.invalidBase64); }
      if (crypto.createHmac('sha1', credential.key).update(hash).digest('hex') !== credential.sad) { return next(errors.unauthorisedHash); }
