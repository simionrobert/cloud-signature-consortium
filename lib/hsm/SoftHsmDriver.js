...
class SoftHSMDriver {
    ...
    sign(hash, pin) {
        ...
        if (slot.flags & graphene.SlotFlag.TOKEN_PRESENT) {
            ...
            logger.log("Signature RSA-SHA1:", signature.toString("hex")); // Signature RSA-SHA1: 6102a66dc0d97fadb5...

            // verify content
            const verify = session.createVerify("SHA1_RSA_PKCS", keys.publicKey);
            verify.update(hash);
            const verify_result = verify.final(signature);
            logger.log("Signature RSA-SHA1 verify:", verify_result);      // Signature RSA-SHA1 verify: true

            session.logout();
            session.close();
        }
        else {
            logger.error("Slot is not initialized");
        }
