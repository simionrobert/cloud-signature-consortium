...
router.post(
  ...
  function(req, res, next) {
    ...
    if (credentialID === undefined || credentialID === null) {
      return next(errors.invalidCredentialIDFormat);
    }
    if (
      certificates !== 'none' &&
      certificates !== 'single' &&
...
router.post(
  ...
  function(req, res, next) {
    ...
    if (numSignatures === undefined || !validator.isNumeric(numSignatures)) {
      return next(errors.missingNumSignatures);
    }
    if (numSignatures < 1) {
      return next(errors.invalidNumSignatures);
    }
    let err = false;
    hashes.forEach(hash => {
    ...
    Credential.findOne({ credentialID: credentialID }, function(err, doc) {
      ...
      if (doc.status === 'disabled') {
        return next(errors.disabledCredentialID);
      }
      if (doc.multisign < numSignatures) {
        return next(errors.tooHighNumSignatures);
      }

      // verify pin
      let userCredentialObject = user.credentials.find(
        x => x.credentialID === credentialID
      );
      if (userCredentialObject === undefined)
        return next(errors.invalidCredentialID);
...
router.post(
  ...
  function(req, res, next) {
    ...
    Credential.findOne({ credentialID: credentialID }, function(err, doc) {
      ...
      if (doc.key.status == 'disabled') {
        return next(errors.disabledSigningKey);
      }
      ...
      User.updateOne(
        ...
        function(err) {
          if (err) {
            return next(errors.databaseError);
          }
