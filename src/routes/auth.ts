    ...
    try {
      ...
      if (rememberMe === true) {
        ...
        crypto.randomBytes(48, function(err, buffer) {
          ...
          Token.updateOne(
            { user_id: user._id, type: 'refresh_token' },
            {
        ...
        if (refresh_token != null) {
          ...
          crypto.randomBytes(48, function(err, buffer) {
            ...
            Token.updateOne(
              { user_id: user._id, type: 'access_token' },
              {
...
router.post(
  ...
  function(req, res, next) {
    ...
    if (
      ...
      return next(errors.invalidTokenHint);

    if (hint === undefined) {
      // update user with the access token and timestamp
      Token.findOneAndDelete(
        { user_id: user._id, value: utils.hash(token), type: 'access_token' },
        (err, doc) => {
          if (err) return next(errors.databaseError);
          if (!doc) {
            ...
            Token.findOneAndDelete(
              {
                user_id: user._id,
                value: utils.hash(token),
                type: 'refresh_token'
              },
              (err, doc) => {
                if (err) return next(errors.databaseError);
                if (!doc) {
                  return next(errors.invalidTokenParameter);
                }
